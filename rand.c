...
24/4/2026 - Peter Miller, fixed bug in that "t =4294584393UL*z+c;" calculated the multiply and addition as uint32_t so t>>32 was always zero (at least with gcc 15.2.0 for Windows). 
			As any 2 generators alone would pass the Dieharder tests having one of the 3 only partly working would still generate "reasonable" random numbers, but the period would be reduced to 2^96 (7.9e28)? [ at worse to 2^64],
			which given RAND_MAX is 2^31-1 does not appear to be a major issue.
			Also updated ya_srand() to ensure all generators seeded with valid values.

WARNING: rand.h must be included in any file using these functions (if not they could be called as ya_rand(), but RAND_MAX is likely to be wrong!)
If used for cryptographic applications see warning on srand() below.
*/
#include <stdint.h>
#include "rand.h"
...
static uint32_t JKISS(void)
{
 ...
 y^=y>>7;
 y^=y<<22;
 t =(uint64_t)4294584393U*(uint64_t)z+(uint64_t)c;// uint64_t casts ensure maths is done in 64 bits
 c= t>>32;
 z= t;
 return x+y+z;
}
...
{ // seed random number generator, 0 gives the same sequence as you get without calling srand() 
  // warning - this will only give a number of different sequences limited by the size of an unsigned int - ideally the seed needs to be 4 * 32 bit unsigned numbers but this matches the standard C library srand().
 if(seed==0)
	 ...
	 // PMi 24/4/2026 add extra tests to ensure y!=0 & z&c both not =0
	 x=433494437U^(~seed);
	 y=2971215073U+(~seed*257U);// *prime number
	 if(y==0) y=2971215073U;// y is the only one that cannot be 0
	 z=2147483647U^(seed*65537U);// *prime number
	 c=2521008887U+(seed*1442968193U);// *prime number 1442968193 = 1,442,968,193
	 if(z==0 && c==0) z=2147483647U;// was probably never possible (given both are calculated from seed in very different ways), but lets be sure.
 	}
}
